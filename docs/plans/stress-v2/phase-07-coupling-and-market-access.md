...
| Item | Location |
|------|----------|
| `ExternalPortfolioAdjuster` | `src/lic_dsf/stress_v2/external_portfolio.py` |
| `MarketAccessAddon` | `src/lic_dsf/stress_v2/market_access.py` |
| `ComboMarketCost` | `src/lic_dsf/stress_v2/market_access.py` |
| Coupled runner orchestration | `src/lic_dsf/stress_v2/runner/coupled.py` |
...
| Gap | Fix |
|-----|-----|
| `ext_r86` API always zero | Wire Phase 3 gap into Phase 4 `AbsoluteResidualPolicy` |
| B2 split always `capped` | Use `AbsoluteResidualPolicy` per spec |
| B2 Output 3-1 2026 PV ~1e-3 drift | Market add.int + dual-block timing |
...
6. Run full Output 3-1 B2 probe catalog (all 4 indicators × 11 years).

## Differential testing

| Target | Probes |
|--------|--------|
| B2 Output 3-1 | All `output_31_probes` with B2 label |
| B2 Output 3-2 | B2 public rows |
| B6 external | B-sheet ratios + Output 3-1 B6 |
| B5 external | LC-NR PV change vs baseline on B-sheet |
| `PV_ResFin-add.int.cost - mkt` | Interest rows in shock window |
| Coupling | Public fill USD share vs external R86 (sanity) |
...
- [ ] Output 3-1 B2 full catalog at `1e-6`
- [ ] B6 passes without reading `PV_Base-add.cost.mkt` from workbook
- [ ] B5/B6 external ratios reflect LC-NR FX revaluation
- [ ] B2 uses `AbsoluteResidualPolicy` with non-zero `ext_r86` when coupled
- [ ] No `workbook_path` argument on standard runners
...
Remove `load_combo_additional_borrowing_interest` from production runner path;
keep loader only for parity debugging if needed.
